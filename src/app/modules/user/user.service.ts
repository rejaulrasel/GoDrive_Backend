...
async function getUserForRecoverAccountFromDb(
  email: string,
  next: NextFunction,
) {
  try {
...
async function recoverAccountFromDb(
  payload: { token: string; newPassword: string },
  next: NextFunction,
) {
  try {
    const decoded = verifyTokenSync(
      payload.token,
      config.jwt_access_token as string,
    );
...
async function updateSpecificUserIntoDb(
...
async function changeUserRoleIntoDb(
